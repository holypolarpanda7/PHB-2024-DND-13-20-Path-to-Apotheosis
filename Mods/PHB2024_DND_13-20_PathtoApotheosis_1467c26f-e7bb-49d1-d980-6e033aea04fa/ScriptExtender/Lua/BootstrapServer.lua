...
-- =====================================================================
-- Level 7-9 spell riders (spells generated by
-- Scripts/generate_level79_spells.py; designs in docs/SPELLS-7-9.md).
--
-- Finger of Death: a humanoid killed while carrying the
-- APO_FINGER_OF_DEATH mark rises as a zombie serving the caster's side.
-- Clone: a character downed while carrying APO_CLONE is fully revived
-- once; the clone is consumed.
-- =====================================================================

local FINGER_OF_DEATH_MARK = "APO_FINGER_OF_DEATH"
local FINGER_ZOMBIE_TEMPLATE = "c2a2c269-ede8-4887-99f1-e0c044cc0c75" -- AnimateDead zombie
local HERO_FACTION = "a1542c81-6895-929e-4522-10ce218bb360"           -- Hero (fights for party)

Ext.Osiris.RegisterListener("Died", 1, "after", function(character)
    local ok, err = pcall(function()
        if Osi.HasActiveStatus(character, FINGER_OF_DEATH_MARK) ~= 1 then return end
        local x, y, z = Osi.GetPosition(character)
        if not x then return end
        local zombie = Osi.CreateAt(FINGER_ZOMBIE_TEMPLATE, x + 1, y, z + 1, 0, 0, "")
        if zombie and zombie ~= "" then
            pcall(Osi.SetFaction, zombie, HERO_FACTION)
            Log.Info("Finger of Death: " .. tostring(character) .. " rises as zombie " .. tostring(zombie))
        end
    end)
    if not ok then
        Log.Error("Finger of Death rider error: " .. tostring(err))
    end
end)

local CLONE_MARK = "APO_CLONE"

Ext.Osiris.RegisterListener("StatusApplied", 4, "after", function(object, status, _, _)
    local ok, err = pcall(function()
        if status ~= "DOWNED" then return end
        if Osi.HasActiveStatus(object, CLONE_MARK) ~= 1 then return end
        Osi.RemoveStatus(object, CLONE_MARK)
        Ext.Timer.WaitFor(300, function()
            local okRevive, errRevive = pcall(function()
                Osi.Resurrect(object)
                Log.Info("Clone: " .. tostring(object) .. " restored to full strength; clone consumed")
            end)
            if not okRevive then
                Log.Error("Clone revive error: " .. tostring(errRevive))
            end
        end)
    end)
    if not ok then
        Log.Error("Clone rider error: " .. tostring(err))
    end
end)
